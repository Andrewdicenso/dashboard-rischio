...
class DatabaseAziendale:
    ...
    def crea_utente(self, email, password, ruolo="user", azienda=None):
        ...
        try:
            with self._get_conn() as conn:
                ...
                email_enc = self.vault.encrypt_data(email.strip().lower())
    ...
    def verifica_utente(self, email, password):
        """
        Verifica le credenziali di un utente usando bcrypt.
        """
        utente = self.get_utente_by_email(email)
        if utente:
            # Recuperiamo l'hash salvato nel DB
            stored_hash = utente.get("password_hash").encode()
            # Verifichiamo se la password inserita corrisponde all'hash
            if bcrypt.checkpw(password.encode(), stored_hash):
                return utente
        return None

    def get_utente_by_email(self, email):    
        """Recupera un utente a partire dall'email in chiaro analizzando la tabella."""
        try:
            ...
            for row in rows:
                try:
                    decrypted_email = self.vault.decrypt_data(row[1]).strip().lower()
                    ...
                    continue  # Salta riga se ci sono vecchi dati corrotti non decifrabili
...
# Istanza globale centralizzata per l'applicazione
